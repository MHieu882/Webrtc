...
const register = async (req, res) => {
  try {
    ...
    if (email) {
      res.render("register", { message: "Email is already exists!" });
    } else {
      ...
      await users.save();
      res.render("login", {
        message: "Your Registration has beend Completed!",
      });
    }
  } catch (error) {
    console.log(error.message);
...
const changePassword = async (req, res) => {
  try {
    ...
    if (!checkPass) {
      avt = req.sesion.avatar;
      res.render("ChangePass", { avt, message: "Password is Incorrect!" });
    } else if (req.body.newpassword != req.body.newpassword2) {
      res.render("ChangePass", { avt, message: "New Password 2 is not Same!" });
    } else {
...
const OtpPage = async (req, res) => {
  ...
  if (Date.parse(data.expiredAt) < Date.parse(DateNow)) {
    ...
    return res.render("ResetPassword", {
      message: "Your OTP already expried Please send request again",
    });
